...
plt.title("A graph of Ages against Developers")
...
avg_scores = file.groupby('department')['score'].mean().reset_index()
plt.bar(avg_scores['department'], avg_scores['score'], color='skyblue')
...
plt.plot (date,cat, label= 'Category')

#by Amount
amount = file.Amount
plt.plot (date, amount, label= 'Amount')
